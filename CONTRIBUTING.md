...
4. Do **not** commit `.env`, `.env.stack`, `.secrets`, `secrets/`, `acme.json`, or generated `config/*`.
...
- Secrets only via `.env` / server `.secrets` / local `secrets/<timestamp>` archive, never in the repo.
